...
class App:
    def __init__(self, root):
        ...
        # 步骤容器
        self.steps_container = ttk.Frame(self.root)
    ...
    def add_step(self):
        ...
        params_frame = ttk.Frame(self.steps_frame)
    ...
    def update_step_params(self, index):
        ...
        if action == 'find_and_click':
            ...
            combo_image.grid(row=0, column=1, columnspan=3, sticky="we")
            self.steps[index]['params_widgets']['image_path'] = combo_image

            # X/Y偏移
            entry_dx = ttk.Entry(self.steps[index]['params_frame'], width=10)
            entry_dx.insert(0, '0')
            entry_dx.grid(row=1, column=1, sticky="w")
            self.steps[index]['params_widgets']['d_x'] = entry_dx

            entry_dy = ttk.Entry(self.steps[index]['params_frame'], width=10)
            entry_dy.insert(0, '0')
            entry_dy.grid(row=1, column=3, sticky="w")
            self.steps[index]['params_widgets']['d_y'] = entry_dy

            # 点击次数
            entry_clicks_1 = ttk.Entry(self.steps[index]['params_frame'], width=10)
            ...
            entry_text = ttk.Entry(self.steps[index]['params_frame'], width=50)
            ...
            # 坐标参数
            entry_x = ttk.Entry(self.steps[index]['params_frame'], width=10)
            entry_x.insert(0, '0')
            entry_x.grid(row=0, column=1)
            self.steps[index]['params_widgets']['X'] = entry_x

            entry_y = ttk.Entry(self.steps[index]['params_frame'], width=10)
            entry_y.insert(0, '0')
            entry_y.grid(row=0, column=3)
            self.steps[index]['params_widgets']['Y'] = entry_y

            # 滚动参数
            entry_scroll_times = ttk.Entry(self.steps[index]['params_frame'], width=10)
            ...
            entry_scroll_dist = ttk.Entry(self.steps[index]['params_frame'], width=10)
            ...
            # 点击次数
            entry_clicks_2 = ttk.Entry(self.steps[index]['params_frame'], width=10)
